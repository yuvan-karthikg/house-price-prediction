...
if uploaded_file is not None:
    ...
    # Train or Load model
    def train_ann_model(X_train, y_train, X_test, y_test, model_path="house_ann_model.h5"):
        if os.path.exists(model_path):
            st.info("📦 Loading saved model...")
            model = load_model(model_path)
        else:
            st.info("🧠 Training new ANN model...")
    ...
    # Optional retrain
    if st.checkbox("🔁 Retrain the model from scratch"):
        if os.path.exists("house_ann_model.h5"):
            os.remove("house_ann_model.h5")
            st.warning("⚠️ Old model removed. A new model will be trained now.")
