...
history = model.fit(X_train_scaled, y_train, epochs=200, validation_split=0.1, verbose=0, callbacks=[early_stop])
...
actual = np.expm1(y_test)      # Inverse log
...
# Scatter plot
fig, ax = plt.subplots()
sns.scatterplot(x=actual, y=y_pred, ax=ax)
...
r2_rf = r2_score(actual, y_rf_pred)
st.write(f"**R² using Random Forest (Baseline):** {r2_rf:.2f}")
...
if st.button('Predict'):
    pred_log = model.predict(input_scaled).flatten()[0]
